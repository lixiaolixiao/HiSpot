...
class MinimumFlowInterceptionModel(Flow):
    def __init__(self, num_path, num_vector, num_choice, path_vector, path_flow, intercept_e, solver):
        super().__init__(num_path, num_vector, path_vector, solver)
        self.num_choice = num_choice
        self.path_flow = path_flow
        self.intercept_e = intercept_e
        self.intercept_flow = (1 - self.intercept_e) * sum(self.path_flow)
    ...
    def prob_solve(self):
        ...
        prob += pulp.lpSum(self.path_flow[p] * xp[p] for p in range(self.num_path)) >= self.intercept_flow
